...
UIScrollViewDelegate, UINavigationControllerDelegate,RCTKInputBarControlDelegate,RCConnectionStatusChangeDelegate,UIAlertViewDelegate,PLPlayerDelegate>
        ...
        [[RCIMClient sharedRCIMClient]
         ...
         success:^{
             dispatch_async(dispatch_get_main_queue(), ^{
                 ...
                 self.player = [PLPlayer playerWithURL:[NSURL URLWithString:self.contentURL] option:option];
...
- (void)initializedLiveSubViews {
    _liveView = self.player.playerView;
...
-(void)changeCrossOrVerticalscreen:(BOOL)isVertical{
    ...
    if (!isVertical) {
        ...
        self.player.playerView.frame = CGRectMake(0, 0, self.view.frame.size.width, self.view.frame.size.height - self.contentView.frame.size.height);
    }
    float inputBarOriginY = self.conversationMessageCollectionView.bounds.size.height + 30;
    float inputBarOriginX = self.conversationMessageCollectionView.frame.origin.x;
    float inputBarSizeWidth = self.contentView.frame.size.width;
